...
public class ImageUtils {
    ...
    static {
        try {
            ...
            LOGGER.mo6300w("Native library not found, native RGB -> YUV conversion may be unavailable.", new Object[0]);
        }
    }
    ...
    public static void saveBitmap(Bitmap bitmap, String filename) {
        ...
        LOGGER.mo6294i("Saving %dx%d bitmap to %s.", Integer.valueOf(bitmap.getWidth()), Integer.valueOf(bitmap.getHeight()), root);
        File myDir = new File(root);
        if (!myDir.mkdirs()) {
            LOGGER.mo6294i("Make dir failed", new Object[0]);
        }
    ...
    public static void convertYUV420SPToARGB8888(byte[] input, int width, int height, int[] output) {
        if (useNativeConversion) {
            try {
                ...
                LOGGER.mo6300w("Native YUV420SP -> RGB implementation not found, falling back to Java implementation", new Object[0]);
    ...
    public static void convertYUV420ToARGB8888(byte[] yData, byte[] uData, byte[] vData, int width, int height, int yRowStride, int uvRowStride, int uvPixelStride, int[] out) {
        ...
        if (useNativeConversion) {
            try {
                ...
                LOGGER.mo6300w("Native YUV420 -> RGB implementation not found, falling back to Java implementation", new Object[0]);
                useNativeConversion = false;
            }
        }
        int yp2 = 0;
    ...
    public static Matrix getTransformationMatrix(int srcWidth, int srcHeight, int dstWidth, int dstHeight, int applyRotation, boolean maintainAspectRatio) {
        ...
        if (applyRotation != 0) {
            if (applyRotation % 90 != 0) {
                LOGGER.mo6300w("Rotation of %d % 90 != 0", Integer.valueOf(applyRotation));
            }
